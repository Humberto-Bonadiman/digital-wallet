...
var ValidateUser = /** @class */ (function () {
    ...
    ValidateUser.prototype.verifyIfEmpty = function (req, res, next) {
    ...
    ValidateUser.prototype.validateUsername = function (req, res, next) {
        var username = req.body.username;
        if (username.length <= 3) {
            return res.status(StatusCode_1["default"].BAD_REQUEST)
                .json({ message: '"username" length must be at least 3 characters long' });
        }
        var regexUsername = /^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/;
        var isValidUsername = regexUsername.test(username);
    ...
    ValidateUser.prototype.verifyPassword = function (req, res, next) {
        var password = req.body.password;
        if (password.length < 8) {
            return res.status(StatusCode_1["default"].BAD_REQUEST)
                .json({ message: '"password" length must be at least 8 characters long' });
        }
        var doesItHaveNumber = /[0-9]/.test(password);
        var doesItHaveUppercaseLetter = /[A-Z]/.test(password);
